...
export default {
  ...
  computed: {
    ...
    bannerBgStyle() {
      ...
      else if (bannerBg.includes('background:')) {
        // 自定义背景样式
        return bannerBg
      }
      else if (bannerBg.includes('.')) {
        // 大图
  ...
  beforeMount() {
    this.isMQMobile
      = window.innerWidth < MOBILE_DESKTOP_BREAKPOINT // vupress在打包时不能在beforeCreate(),created()访问浏览器api（如window）
  },
  mounted() {
    ...
    if (this.hasFeatures) {
      window.addEventListener('resize', () => {
        this.isMQMobile
          = window.innerWidth < MOBILE_DESKTOP_BREAKPOINT
    ...
    >
      ...
      >
        <header class="hero">
          ...
          <p v-if="homeData.actionText && homeData.actionLink" class="action">
            <NavLink class="action-button" :item="actionLink" />
          </p>
        </header>

        <!-- PC端features块 s -->
        <div v-if="hasFeatures && !isMQMobile" class="features">
          <div
      ...
      <div v-if="hasFeatures" v-show="isMQMobile" class="slide-banner">
        <div class="banner-wrapper">
          <div ref="slide" class="slide-banner-scroll">
    ...
    <div class="main-layout-mask">
      <MainLayout>
        <template #mainLeft>
          ...
          >
            <PostList :current-page="currentPage" :per-page="perPage" />
            <Pagination
